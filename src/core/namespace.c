...
/* ProtectSystem=full includes ProtectSystem=true */
...
static int append_mounts(BindMount **p, char **strv, MountMode mode) {
        ...
        STRV_FOREACH(i, strv) {
                ...
                append_mount(p, *i, mode, (*p)->ignore);
        }
...
static int append_target_mounts(BindMount **p, const char *root_directory, const TargetMount *mounts, const size_t size) {
        ...
        for (i = 0; i < size; i++) {
                ...
                append_mount(p, path, m->mode, m->ignore);
        }
                ...
                ProtectSystem protect_system)
{
        ...
        unsigned protect_home_cnt = 0;

        switch (protect_system) {
        case PROTECT_SYSTEM_STRICT:
                protect_system_cnt = ELEMENTSOF(protect_system_strict_table);
                break;
        case PROTECT_SYSTEM_YES:
                protect_system_cnt = ELEMENTSOF(protect_system_yes_table);
                break;
        case PROTECT_SYSTEM_FULL:
                protect_system_cnt = ELEMENTSOF(protect_system_full_table);
                break;
        default:
                break;
        }
        ...
        if (protect_home != PROTECT_HOME_NO)
                protect_home_cnt = ELEMENTSOF(protect_home_yes_table);
